...
class TestSaveFile(unittest.TestCase):
    ...
    def test_save_small_multibyte_file(self):
        """Save a small multibyte UTF-8 file."""
    ...
    def test_save_63kb_multibyte_succeeds(self):
        """Save 63KB of multibyte UTF-8 text."""
    ...
    def test_save_65kb_multibyte_succeeds(self):
        """Save 65KB of multibyte UTF-8 text — no crash.

        Primary regression test: the old code used character count for
        buffer allocation, causing a buffer overrun when multibyte
        characters pushed the byte count past 64KB.
        """
    ...
    def test_save_65kb_ascii_succeeds(self):
        """Save 65KB of ASCII text — control case."""
    ...
    def test_multibyte_spanning_64kb_boundary(self):
        """Place a 4-byte char at the 64KB boundary — no corruption."""
        filepath = os.path.join(self.tmpdir, "boundary.txt")
        ascii_prefix = "A" * (65534)
        boundary_char = "\U0001F600"
        content = ascii_prefix + boundary_char
        save_file(filepath, content)
        with open(filepath, "r", encoding="utf-8") as f:
            self.assertEqual(f.read(), content)
